...
import { HiMenuAlt2, HiMenuAlt3, HiOutlineLogout, HiOutlineUserCircle, HiPlay, HiBookOpen, HiChartPie, HiColorSwatch, HiHome } from "react-icons/hi";
...
const Dashboard = () => {
  ...
  return (
    ...
    <aside className={`h-full bg-slate-100 w-3/4 md:w-2/4 lg:w-1/4 absolute z-20 top-0 left-0 flex flex-col ${open ? '' : '-translate-x-full'} lg:translate-x-0 transition-all duration-300 p-2`}>
      <ul className='space-y-3'>
        ...
        <li className='w-full flex flex-col place-items-center space-y-2'>
          ...
          <h1 className='text-xl lg:text-2xl font-semibold'>{parsedDataUser.providerData[0].displayName}</h1>
          <p className='text-sm w-full text-center'>{parsedDataUser.providerData[0].email}</p>
    ...
    <div className='w-full lg:w-3/4 absolute top-[50px] lg:top-0 right-0 px-4 lg:py-2'>
      <Outlet/>
    </div>
    </>
  )
